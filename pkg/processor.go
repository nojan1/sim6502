...
// SetVector can be used to set one of the system vectors.
// The value of vector should be one of:
func (p *Processor) SetVector(vector uint16, value uint16) *Processor {
	p.memory.Write(uint16(vector), uint8(value&0xff))
	p.memory.Write(uint16(vector+1), uint8(value>>8))
	return p
}
...
func (p *Processor) RunFrom(addr uint16) (err error) {
	...
	for {
		...
		if p.reset {
			...
			// Now it went high, we load the reset vector
		...
		if p.clock > 0 {
			...
			// We've already exceeded the time we should have reached, probably due to context
			// switching of the process, so just keep rolling forward, we should catch up unless
			// an unrealistic clock is set
		}
